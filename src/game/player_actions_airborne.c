...
static s32 act_wall_slide(struct PlayerState *m) {
    ...
    set_player_animation(m, CHAR_ANIM_START_WALLKICK);
...
u32 common_air_action_step(struct PlayerState *m, u32 landAction, s32 animation, u32 stepArg) {
    ...
    switch (stepResult) {
        case AIR_STEP_NONE:
            ...
            } else if (((animation == CHAR_ANIM_JUMP_WITH_LIGHT_OBJ) && (m->vel[1] < 0) && (m->input & INPUT_A_DOWN) && (curChar == 2) && !(m->flags & PLAYER_WING_CAP)) && (m->playerObj->header.gfx.animInfo.animID == CHAR_ANIM_RUN_WITH_LIGHT_OBJ || is_anim_at_end(m))) {
                set_player_anim_with_accel(m, CHAR_ANIM_RUN_WITH_LIGHT_OBJ, 0x00095000);
                if (is_anim_at_end(m)) {
                    set_anim_to_frame(m, 0);
                }
            }
...
s32 act_triple_jump(struct PlayerState *m) {
    ...
    common_air_action_step(m, ACT_TRIPLE_JUMP_LAND, CHAR_ANIM_TRIPLE_JUMP, LEDGE_GRAB_CHECK_MASK);
#ifdef RUMBLE_FEEDBACK
...
s32 act_riding_shell_air(struct PlayerState *m) {
    play_player_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
    set_player_animation(m, CHAR_ANIM_JUMP_RIDING_SHELL);
...
s32 act_twirling(struct PlayerState *m) {
    ...
    set_player_animation(m, ANIM_TWIRL(m->heldObj, CHAR_ANIM_PICK_UP_LIGHT_OBJ, (m->actionArg == 0 ? CHAR_ANIM_START_TWIRL : CHAR_ANIM_TWIRL)));
...
s32 act_air_throw(struct PlayerState *m) {
    ...
    set_player_animation(m, CHAR_ANIM_THROW_LIGHT_OBJECT);
    update_air_without_turn(m);
...
s32 act_water_jump(struct PlayerState *m) {
    ...
    switch (perform_air_step(m, AIR_STEP_CHECK_LEDGE_GRAB)) {
        ...
        case AIR_STEP_GRABBED_LEDGE:
            set_player_animation(m, CHAR_ANIM_IDLE_ON_LEDGE); // This was missing on JP
...
s32 act_hold_water_jump(struct PlayerState *m) {
    ...
    play_player_sound(m, SOUND_ACTION_WATER_JUMP, 0);
    set_player_animation(m, CHAR_ANIM_JUMP_WITH_LIGHT_OBJ);
...
s32 act_ground_pound(struct PlayerState *m) {
    ...
    if (m->actionState == 0) {
        ...
        set_player_animation(m, m->actionArg == 0 ? CHAR_ANIM_START_GROUND_POUND
        ...
        set_player_animation(m, CHAR_ANIM_GROUND_POUND);
...
u32 common_air_knockback_step(struct PlayerState *m, u32 landAction, u32 hardFallAction, s32 animation,
    ...
    switch (stepResult) {
        ...
        case AIR_STEP_HIT_WALL:
            set_player_animation(m, CHAR_ANIM_BACKWARD_AIR_KB);
            player_bonk_reflection(m, FALSE);
...
s32 act_getting_blown(struct PlayerState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        ...
        case AIR_STEP_HIT_WALL:
            set_player_animation(m, CHAR_ANIM_AIR_FORWARD_KB);
            player_bonk_reflection(m, FALSE);
...
s32 act_forward_rollout(struct PlayerState *m) {
    ...
    switch (perform_air_step(m, LEDGE_GRAB_CHECK_MASK)) {
        case AIR_STEP_NONE:
            if (m->actionState == 1) {
                if (set_player_animation(m, CHAR_ANIM_FORWARD_SPINNING) == 4) {
                    play_sound(SOUND_ACTION_SPIN, m->playerObj->header.gfx.cameraToObject);
                }
            } else {
                set_player_animation(m, CHAR_ANIM_GENERAL_FALL);
...
s32 act_backward_rollout(struct PlayerState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_NONE:
            if (m->actionState == 1) {
                if (set_player_animation(m, CHAR_ANIM_BACKWARD_SPINNING) == 4) {
                    play_sound(SOUND_ACTION_SPIN, m->playerObj->header.gfx.cameraToObject);
                }
            } else {
                set_player_animation(m, CHAR_ANIM_GENERAL_FALL);
...
s32 act_butt_slide_air(struct PlayerState *m) {
    ...
    set_player_animation(m, CHAR_ANIM_SLIDE);
    return FALSE;
}
...
s32 act_hold_butt_slide_air(struct PlayerState *m) {
    ...
    set_player_animation(m, CHAR_ANIM_SLIDING_ON_BOTTOM_WITH_LIGHT_OBJ);
    return FALSE;
}
...
s32 act_lava_boost(struct PlayerState *m) {
    ...
    set_player_animation(m, CHAR_ANIM_FIRE_LAVA_BURN);
...
s32 act_jump_kick(struct PlayerState *m) {
    ...
    if (m->actionState == 0) {
        ...
        set_player_animation(m, CHAR_ANIM_AIR_KICK);
        m->actionState = 1;
    }
...
s32 act_shot_from_cannon(struct PlayerState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_NONE:
            set_player_animation(m, CHAR_ANIM_AIRBORNE_ON_STOMACH);
...
s32 act_flying(struct PlayerState *m) {
    ...
    if (m->actionState == 0) {
        if (m->actionArg == 0) {
            set_player_animation(m, CHAR_ANIM_FLY_FROM_CANNON);
        } else {
            set_player_animation(m, CHAR_ANIM_FORWARD_SPINNING_FLIP);
...
s32 act_riding_hoot(struct PlayerState *m) {
    ...
    if (m->actionState == 0) {
        ...
        if (is_anim_at_end(m)) {
            set_player_animation(m, CHAR_ANIM_HANG_ON_OWL);
            m->actionState = 1;
        }
    }
...
s32 act_flying_triple_jump(struct PlayerState *m) {
    ...
    play_player_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_YAHOO);
    if (m->actionState == 0) {
        set_player_animation(m, CHAR_ANIM_TRIPLE_JUMP_FLY);
