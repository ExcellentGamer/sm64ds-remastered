...
s32 act_start_sleeping(struct PlayerState *m) {
    ...
    switch (m->actionState) {
        case 0:
            animFrame =
            set_player_animation(m, CHAR_ANIM_START_SLEEP_IDLE);
            break;

        case 1:
            animFrame =
            set_player_animation(m, CHAR_ANIM_START_SLEEP_SCRATCH);
            break;

        case 2:
            animFrame =
            set_player_animation(m, CHAR_ANIM_START_SLEEP_YAWN);
            m->playerBodyState->eyeState = PLAYER_EYES_HALF_CLOSED;
            break;

        case 3:
            animFrame = set_player_animation(m, CHAR_ANIM_START_SLEEP_SITTING);
            m->playerBodyState->eyeState = PLAYER_EYES_HALF_CLOSED;
            break;
    }
...
s32 act_sleeping(struct PlayerState *m) {
    ...
    switch (m->actionState) {
        case 0:
            animFrame = set_player_animation(m, CHAR_ANIM_SLEEP_IDLE);
        ...
        case 1:
            if (set_player_animation(m, CHAR_ANIM_SLEEP_START_LYING) == 18) {
        ...
        case 2:
            animFrame = set_player_animation(m, CHAR_ANIM_SLEEP_LYING);
...
s32 act_waking_up(struct PlayerState *m) {
    ...
    set_player_animation(m, !m->actionArg ? CHAR_ANIM_WAKE_FROM_SLEEP : CHAR_ANIM_WAKE_FROM_LYING);
...
s32 act_shivering(struct PlayerState *m) {
    ...
    switch (m->actionState) {
        case 0:
            animFrame = set_player_animation(m, CHAR_ANIM_SHIVERING_WARMING_HAND);
            if (animFrame == 49) {
        ...
        case 1:
            animFrame = set_player_animation(m, CHAR_ANIM_SHIVERING);
        ...
        case 2:
            set_player_animation(m, CHAR_ANIM_SHIVERING_RETURN_TO_IDLE);
            if (is_anim_past_end(m)) {
...
s32 act_coughing(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    animFrame = set_player_animation(m, CHAR_ANIM_COUGHING);
...
s32 act_hold_idle(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    set_player_animation(m, CHAR_ANIM_IDLE_WITH_LIGHT_OBJ);
    return FALSE;
}
...
s32 act_hold_heavy_idle(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    set_player_animation(m, CHAR_ANIM_IDLE_HEAVY_OBJ);
    return FALSE;
}
...
s32 act_in_quicksand(struct PlayerState *m) {
    ...
    if (m->quicksandDepth > 70.0f) {
        ...
        set_player_animation(m, CHAR_ANIM_IDLE_IN_QUICKSAND);
    }
...
s32 act_crouching(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    set_player_animation(m, CHAR_ANIM_CROUCHING);
    return FALSE;
}
...
s32 act_panting(struct PlayerState *m) {
    ...
    if (set_player_animation(m, CHAR_ANIM_WALK_PANTING) == 1) {
...
s32 act_hold_panting_unused(struct PlayerState *m) {
    ...
    set_player_animation(m, CHAR_ANIM_WALK_PANTING);
    stationary_ground_step(m);
    m->playerBodyState->eyeState = PLAYER_EYES_HALF_CLOSED;
    return FALSE;
}
...
s32 act_start_crouching(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    set_player_animation(m, CHAR_ANIM_START_CROUCHING);
    if (is_anim_past_end(m)) {
...
s32 act_stop_crouching(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    set_player_animation(m, CHAR_ANIM_STOP_CROUCHING);
    if (is_anim_past_end(m)) {
        set_player_action(m, ACT_IDLE, 0);
    }
    return FALSE;
...
s32 act_start_crawling(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    set_player_animation(m, CHAR_ANIM_START_CRAWLING);
    if (is_anim_past_end(m)) {
...
s32 act_stop_crawling(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    set_player_animation(m, CHAR_ANIM_STOP_CRAWLING);
    if (is_anim_past_end(m)) {
        set_player_action(m, ACT_CROUCHING, 0);
    }
    return FALSE;
...
s32 act_long_jump_land_stop(struct PlayerState *m) {
    ...
    landing_step(m, !m->playerObj->oPlayerLongJumpIsSlow ? CHAR_ANIM_CROUCH_FROM_FAST_LONGJUMP
...
s32 act_twirl_land(struct PlayerState *m) {
    ...
    stationary_ground_step(m);
    CHECK(m->heldObj, set_player_animation(m, CHAR_ANIM_TWIRL_LAND));
    if (m->angleVel[1] > 0) {
        m->angleVel[1] -= 0x400;
        if (m->angleVel[1] < 0) {
            m->angleVel[1] = 0;
